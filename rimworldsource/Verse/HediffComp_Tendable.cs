...
namespace Verse
{
	public class HediffComp_Tendable : HediffComp
	{
		...
		public override string CompTipStringExtra
		{
			get
			{
				...
				if (this.IsTended && !this.parent.IsOld())
				{
					...
					{
						if (this.parent.Part != null && this.parent.Part.depth == BodyPartDepth.Inside)
						{
							if (this.parent.IsTendedWell())
							{
		...
		public override void CompTended(float quality, int batchPosition = 0)
		{
			...
			if (batchPosition == 0)
			{
				...
				if (typeof(Hediff_Injury).IsAssignableFrom(this.parent.GetType()))
				{
					if (num < 1f)
					{
				...
				MoteThrower.ThrowText(this.parent.pawn.DrawPos, text, color, 220);
			}
		}
	}
}

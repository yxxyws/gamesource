	...
	public static class GenClosest
	{
		...
		public static Thing RegionwiseBFSWorker(IntVec3 root, ThingRequest req, PathEndMode peMode, TraverseParms traverseParams, Predicate<Thing> validator, Func<Thing, float> priorityGetter, int minRegions, int maxRegions, float maxDistance)
		{
			...
			RegionEntryPredicate entryCondition = delegate(Region from, Region to)
			{
				...
				{
					if (to.portal == null || (peMode != PathEndMode.Touch && peMode != PathEndMode.ClosestTouch))
					{
						return false;
					}
					if (!from.Allows(traverseParams, false))
					{
						return false;
					}
				}
			...
			ProfilerThreadCheck.EndSample();
			RegionTraverser.BreadthFirstTraverse(validRegionAt, entryCondition, regionProcessor, maxRegions);
			return closestThing;
		}

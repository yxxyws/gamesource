	...
	public class Sustainer
	{
		...
		public float CameraDistanceSquared
		{
			get
			{
				...
				return (Find.CameraMap.MapPosition - this.worldRootObject.transform.position.ToIntVec3()).LengthHorizontalSquared;
			}
		}
		...
		public void SustainerUpdate()
		{
			if (!this.Ended)
			{
				...
				{
					if (this.info.Maintenance == MaintenanceType.PerFrame && Time.frameCount > this.lastMaintainFrame + 1)
					{
						this.End();
						return;
					}
			...
			{
				if (this.TimeSinceEnd > this.def.sustainFadeoutTime)
				{
					this.Cleanup();
				}
			}

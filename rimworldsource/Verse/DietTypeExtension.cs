...
namespace Verse
{
	public static class DietTypeExtension
	{
		public static string ToHumanString(this FoodTypeFlags ft)
		{
			...
			if ((ft & FoodTypeFlags.NutritionalDrug) != FoodTypeFlags.None)
			{
				if (text.Length > 0)
				{
					text += ", ";
				}
				text += "FoodTypeFlags_NutritionalDrug".Translate();
			}
			if ((ft & FoodTypeFlags.AnimalKibble) != FoodTypeFlags.None)
			{
				if (text.Length > 0)
				{
					text += ", ";
				}
				text += "FoodTypeFlags_AnimalKibble".Translate();
			}
			return text;
		}
	}

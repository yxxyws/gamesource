	...
	public static class GlowFlooder
	{
		...
		public static void AddFloodGlowFor(CompGlower theGlower)
		{
			...
			while (GlowFlooder.openSet.Count != 0)
			{
				...
				if (Game.isDebugBuild && DebugViewSettings.drawGlow)
				{
		...
		private static void SetGlowGridFromDist(IntVec3 c)
		{
			...
			{
				float to = 1f / (num * num);
				float from = 1f + GlowFlooder.attenLinearSlope * num;
				float b = Mathf.Lerp(from, to, 0.4f);
				colB = GlowFlooder.glower.Props.glowColor * b;
			}

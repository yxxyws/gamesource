...
namespace RimWorld
{
	public class HistoryAutoRecorderGroup : IExposable
	{
		...
		{
			foreach (string current in this.def.historyAutoRecorderDefs)
			{
				HistoryAutoRecorderDef historyAutoRecorderDef = HistoryAutoRecorderDef.Named(current);
				if (historyAutoRecorderDef != null)
				{
		...
		public void DrawGraph(Rect rect, Rect legendRect, Vector2 section, SimpleCurveDrawerStyle curveDrawerStyle, List<CurveMark> marks)
		{
			...
			SimpleCurveDrawer.DrawCurves(this.curves, new Rect(rect.xMin, rect.yMin, rect.width, rect.height), curveDrawerStyle, marks, legendRect);

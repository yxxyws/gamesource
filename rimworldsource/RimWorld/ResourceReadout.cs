...
namespace RimWorld
{
	public class ResourceReadout
	{
		...
		{
			this.RootThingCategories = (
				from cat in DefDatabase<ThingCategoryDef>.AllDefs
		...
		public void ResourceReadoutOnGUI()
		{
			...
			{
				Rect rect = new Rect(7f, 7f, 9999f, 9999f);
				this.DoReadoutSimple(rect);
			}
			else
			{
				Rect rect2 = new Rect(2f, 7f, 9999f, 9999f);
		...
		private void DoReadoutCategorized(Rect rect)
		{
			...
			listing_ResourceReadout.nestLevelPixelWidth = 7f;
			listing_ResourceReadout.lineHeight = 24f;
			listing_ResourceReadout.lineSpacing = 0f;

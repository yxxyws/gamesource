...
namespace RimWorld
{
	public static class Autotests_RandomNumbers
	{
		...
		{
			List<float> source = Autotests_RandomNumbers.RandomFloats(500).ToList<float>();
			if (source.Any((float x) => x < 0f || x > 1f))
			{
				Log.Error("Float out of range.");
			}
			if (source.Any((float x) => x < 0.1f))
			{
				if (source.Any((float x) => (double)x > 0.5 && (double)x < 0.6))
				{
					if (source.Any((float x) => (double)x > 0.9))
					{
						goto IL_C4;
					}
				}
			...
			IL_C4:
			source = Autotests_RandomNumbers.RandomFloats(1300000).ToList<float>();
			int num = source.Count((float x) => (double)x < 0.1);
			Log.Message("< 0.1 count (should be ~10%): " + (float)num / (float)source.Count<float>() * 100f + "%");
			num = source.Count((float x) => (double)x < 0.0001);
			Log.Message("< 0.0001 count (should be ~0.01%): " + (float)num / (float)source.Count<float>() * 100f + "%");
		}
		[DebuggerHidden]
		private static IEnumerable<float> RandomFloats(int count)
		{
			Autotests_RandomNumbers.<RandomFloats>c__Iterator11B <RandomFloats>c__Iterator11B = new Autotests_RandomNumbers.<RandomFloats>c__Iterator11B();
			<RandomFloats>c__Iterator11B.count = count;
			<RandomFloats>c__Iterator11B.<$>count = count;
			Autotests_RandomNumbers.<RandomFloats>c__Iterator11B expr_15 = <RandomFloats>c__Iterator11B;
			expr_15.$PC = -2;
			return expr_15;
		}

...
namespace RimWorld
{
	public static class PrisonBreakUtility
	{
		...
		public static float InitiatePrisonBreakMtbDays(Pawn pawn)
		{
			...
			return 60f * num * num2 * num3;
		}
		public static bool CanParticipateInPrisonBreak(Pawn pawn)
		{
			return !pawn.Downed && pawn.IsPrisoner && !PrisonBreakUtility.IsEscaping(pawn);
		}
		public static bool IsEscaping(Pawn pawn)
		{
		...
		private static void StartPrisonBreakIn(Room room, List<Pawn> outAllEscapingPrisoners, int sapperThingID, HashSet<Room> participatingRooms)
		{
			...
			IntVec3 exitPoint;
			if (!ExitUtility.TryFindRandomExitSpot(PrisonBreakUtility.escapingPrisonersGroup[0], out exitPoint, TraverseMode.PassDoors))
			{
				return;
			}
			IntVec3 groupUpLoc;
		...
		private static bool TryFindGroupUpLoc(List<Pawn> escapingPrisoners, IntVec3 exitPoint, out IntVec3 groupUpLoc)
		{
			...
			PawnPath pawnPath = PathFinder.FindPath(escapingPrisoners[0].Position, exitPoint, TraverseParms.For(escapingPrisoners[0], Danger.Deadly, TraverseMode.PassDoors, false), PathEndMode.OnCell);
			try
			{
				if (!pawnPath.Found)
				{
		...
		private static bool RoomsAreCloseToEachOther(Room a, Room b)
		{
			...
			PawnPath pawnPath = PathFinder.FindPath(intVec, intVec2, TraverseParms.For(TraverseMode.PassDoors, Danger.Deadly, false), PathEndMode.OnCell);
			bool result;
			try
			{
				if (!pawnPath.Found)
		...
		private static void SendPrisonBreakLetter(List<Pawn> escapingPrisoners)
		{
			...
			{
				stringBuilder.AppendLine("    " + escapingPrisoners[i].LabelBaseShort);
			}

...
namespace RimWorld
{
	public static class SteadyAtmosphereEffects
	{
		...
		private static ModuleBase snowNoise;
		...
		private static void DoCellSteadyEffects(IntVec3 c)
		{
			...
			if (room != null && !room.UsesOutdoorTemperature)
			{
				...
				if (temperature > 0f)
				{
					...
					if (temperature > TemperatureTuning.AutoIgnitionTemperatureRange.min && Rand.Value < TemperatureTuning.AutoIgnitionTemperatureRange.InverseLerpThroughRange(temperature) * 0.25f)
					{
		...
		private static void RollForRainFire()
		{
			...
			if (!Find.RoofGrid.Roofed(building.Position))
			{
				...
				{
					GenExplosion.DoExplosion(building.OccupiedRect().RandomCell, 1.9f, DamageDefOf.Flame, null, null, null, null, null, 0f, false, null, 0f);

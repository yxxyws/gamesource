...
namespace RimWorld
{
	...
	public class WeatherEvent_LightningStrike : WeatherEvent_LightningFlash
	{
		...
		public override void FireEvent()
		{
			...
			GenExplosion.DoExplosion(this.strikeLoc, 1.9f, DamageDefOf.Flame, null, null, null, null, null, 0f, false, null, 0f);
			Vector3 loc = this.strikeLoc.ToVector3Shifted();
			for (int i = 0; i < 4; i++)
			{
				MoteThrower.ThrowSmoke(loc, 1.5f);
				MoteThrower.ThrowMicroSparks(loc);
				MoteThrower.ThrowLightningGlow(loc, 1.5f);
			}

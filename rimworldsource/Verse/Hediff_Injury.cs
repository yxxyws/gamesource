...
namespace Verse
{
	public class Hediff_Injury : HediffWithComps
	{
		...
		private bool TicksAfterNoLongerBleedingPassed
		{
			get
			{
				return this.ticksSinceCreation >= this.TicksAfterNoLongerBleeding;
			}
		}
		...
		public override bool TryMergeWith(Hediff other)
		{
			...
			if (hediff_Injury == null || hediff_Injury.def != this.def || hediff_Injury.Part != base.Part || hediff_Injury.IsTended() || hediff_Injury.IsOld() || this.IsTended() || this.IsOld() || !this.def.injuryProps.canMerge)
			{
				return false;
			}
			this.Severity += hediff_Injury.Severity;

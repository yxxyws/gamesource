...
namespace Verse
{
	public class EditWindow_DebugInspector : EditWindow
	{
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			widgetRow.DoIconToggle(ref this.fullMode, TexButton.InspectModeToggle, "Toggle deep inspection mode for things on the map.", null, null);
			widgetRow.DoIconToggle(ref DebugViewSettings.writeCellContents, TexButton.InspectModeToggle, "Toggle shallow inspection for things on the map.", null, null);
			if (widgetRow.DoTextButton("Visibility", "Toggle what information should be reported by the inspector.", true, false))
			{
				Find.WindowStack.Add(new Dialog_DebugSettingsMenu());
			}
			if (widgetRow.DoTextButton("Column Width +", "Make the columns wider.", true, false))
			{
				this.columnWidth += 20f;
				this.columnWidth = Mathf.Clamp(this.columnWidth, 200f, 1600f);
			}
			if (widgetRow.DoTextButton("Column Width -", "Make the columns narrower.", true, false))
			{
				this.columnWidth -= 20f;
				this.columnWidth = Mathf.Clamp(this.columnWidth, 200f, 1600f);
			}
			...
			listing_Standard.OverrideColumnWidth = this.columnWidth;
			...
			for (int i = 0; i < array2.Length; i++)
			{
				...
				listing_Standard.DoLabel(label);
				listing_Standard.DoGap(-10f);
			}
		...
		private string CurrentDebugString()
		{
			...
			{
				stringBuilder.AppendLine("---");
				stringBuilder.AppendLine(ConceptDecider.GetDebugString());
			}
			...
			if (Game.Mode == GameMode.MapPlaying)
			{
				...
				{
					stringBuilder.AppendLine("---");
					foreach (Pawn current6 in (
						from t in Find.ThingGrid.ThingsAt(Gen.MouseCell())
						where t is Pawn
						select t).Cast<Pawn>())
					{
						stringBuilder.AppendLine(current6.mindState.mentalStateStarter.DebugString());
					}
				}
				...
				if (DebugViewSettings.writeStoryteller)
				{
					...
					stringBuilder.AppendLine("---");
					stringBuilder.AppendLine(Find.StoryWatcher.DebugReadout);
				}
				if (DebugViewSettings.writeListRepairableBldgs)
				{
					stringBuilder.AppendLine("---");
					stringBuilder.AppendLine(ListerBuildingsRepairable.DebugOutput());
				}
				if (DebugViewSettings.writeListFilthInHomeArea)
				{
					stringBuilder.AppendLine("---");
					stringBuilder.AppendLine(ListerFilthInHomeArea.DebugOutput());
				}
				...
				{
					stringBuilder.AppendLine("---");
					stringBuilder.AppendLine(ListerHaulables.DebugOutput());
				}
				...
				if (DebugViewSettings.writeAttackTargets)
				{
					...
					{
						stringBuilder.AppendLine("---");
						stringBuilder.AppendLine("Potential attack targets for " + current10.LabelBaseShort + ":");
				...
				if (intVec.InBounds())
				{
					...
					if (DebugViewSettings.writePathCosts)
					{
						...
						stringBuilder.AppendLine("Real path cost: " + PathGrid.CalculatedCostAt(intVec, false));
					}
					...
					if (DebugViewSettings.drawPreyInfo)
					{
						...
						if (pawn2 != null)
						{
							...
							for (int j = 0; j < thingList.Count; j++)
							{
								...
								{
									stringBuilder.AppendLine("---");
									if (FoodUtility.AcceptablePreyFor(pawn2, pawn3))
									{

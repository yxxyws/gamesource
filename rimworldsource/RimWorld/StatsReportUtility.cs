	...
	public static class StatsReportUtility
	{
		...
		public static void DrawStatsReport(Rect rect, Def def, ThingDef stuff)
		{
			if (StatsReportUtility.cachedDrawEntries.NullOrEmpty<StatDrawEntry>())
			{
				...
				StatsReportUtility.cachedDrawEntries.AddRange(
					from r in StatsReportUtility.StatsToDraw(def, stuff)
		...
		public static void DrawStatsReport(Rect rect, Thing thing)
		{
			if (StatsReportUtility.cachedDrawEntries.NullOrEmpty<StatDrawEntry>())
			{
				...
				StatsReportUtility.cachedDrawEntries.AddRange(
					from r in StatsReportUtility.StatsToDraw(thing)
					where r.ShouldDisplay
					select r);
		...
		{
			StatsReportUtility.<StatsToDraw>c__Iterator114 <StatsToDraw>c__Iterator = new StatsReportUtility.<StatsToDraw>c__Iterator114();
			<StatsToDraw>c__Iterator.def = def;
			<StatsToDraw>c__Iterator.stuff = stuff;
			<StatsToDraw>c__Iterator.<$>def = def;
			<StatsToDraw>c__Iterator.<$>stuff = stuff;
			StatsReportUtility.<StatsToDraw>c__Iterator114 expr_23 = <StatsToDraw>c__Iterator;
			expr_23.$PC = -2;
			return expr_23;
		}
		[DebuggerHidden]
		...
		{
			StatsReportUtility.<StatsToDraw>c__Iterator115 <StatsToDraw>c__Iterator = new StatsReportUtility.<StatsToDraw>c__Iterator115();
			<StatsToDraw>c__Iterator.thing = thing;
			<StatsToDraw>c__Iterator.<$>thing = thing;
			StatsReportUtility.<StatsToDraw>c__Iterator115 expr_15 = <StatsToDraw>c__Iterator;
			expr_15.$PC = -2;
			return expr_15;
		}
		private static void FinalizeCachedDrawEntries(IEnumerable<StatDrawEntry> original, Def def)
		{
			StatsReportUtility.cachedDrawEntries = (
				from sd in original
		...
		private static void DrawStatsWorker(Rect rect, Thing optionalThing)
		{
			...
			{
				StatRequest optionalReq = (optionalThing == null) ? StatRequest.ForEmpty() : StatRequest.For(optionalThing);

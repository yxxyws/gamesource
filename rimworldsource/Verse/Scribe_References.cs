	...
	public static class Scribe_References
	{
		public static void LookReference<T>(ref T refee, string label, bool saveDestroyedThings = false) where T : ILoadReferenceable
		{
			if (Scribe.mode == LoadSaveMode.Saving)
			{
				...
				if (Game.isDebugBuild && thing != null)
				{
		...
		public static bool CheckSaveReferenceToDestroyedThing(Thing th, string label, bool saveDestroyedThings)
		{
			...
			if (th.ThingState == ThingState.Discarded)
			{
				...
				{
					"Trying to save reference to thing ",
					th,
					" which was destroyed with Discard destroy mode. This means that it's not deep-saved anywhere and is no longer managed by anything in the code, so saving its reference will always fail. (even with saveDestroyedThings==true) , label=",
					label
				}));
				Scribe.WriteElement(label, "null");
				return true;

...
namespace RimWorld
{
	public static class RecordsUtility
	{
		...
		public static void Notify_BillDone(Pawn billDoer, List<Thing> products)
		{
			...
			{
				if (products[i].def.IsNutritionSource && products[i].def.ingestible.preferability >= FoodPreferability.Awful)
				{
					billDoer.records.Increment(RecordDefOf.MealsCooked);
				}
				else
				{
					if (RecordsUtility.ShouldIncrementThingsCrafted(products[i]))
					{

...
namespace RimWorld
{
	public sealed class TaleManager : IExposable
	{
		...
		private void CheckCullUnusedTalesWithMaxPerPawnLimit(Tale addedTale)
		{
			...
			for (int j = 0; j < this.tales.Count; j++)
			{
				if (this.tales[j].Unused && this.tales[j].def == addedTale.def && this.tales[j].DominantPawnId == addedTale.DominantPawnId)
				{
					i++;
				}
			}
			while (i > addedTale.def.maxPerPawn)
			{
				...
				for (int k = 0; k < this.tales.Count; k++)
				{
					if (this.tales[k].Unused && this.tales[k].def == addedTale.def && this.tales[k].DominantPawnId == addedTale.DominantPawnId && this.tales[k].AgeTicks > num)
					{
						tale = this.tales[k];
		...
		public Tale GetLatestTale(TaleDef def, Pawn pawn)
		{
			...
			{
				if (this.tales[i].def == def && this.tales[i].DominantPawnId == pawn.thingIDNumber && (tale == null || this.tales[i].AgeTicks < num))
				{
					tale = this.tales[i];
			...
			for (int i = this.tales.Count - 1; i >= 0; i--)
			{
				if (this.tales[i].Unused)
				{
					if (!this.tales[i].def.usableForArt)
					{
						...
						{
							if (this.tales[i].DominantPawnId == pawn.thingIDNumber)
							{
								this.RemoveTale(this.tales[i]);
							}
						}
		...
		public void LogTales()
		{
			...
			IEnumerable<Tale> enumerable = 
				from x in this.tales
				where !x.Unused
				select x;
			IEnumerable<Tale> enumerable2 = 
				from x in this.tales
				where x.def.type == TaleType.Volatile && x.Unused
				select x;
			IEnumerable<Tale> enumerable3 = 
				from x in this.tales
				where x.def.type == TaleType.PermanentHistorical && x.Unused
				select x;
			IEnumerable<Tale> enumerable4 = 
				from x in this.tales
		...
		public void LogTaleInterestSummary()
		{
			...
			TaleDef def;
			Func<TaleDef, float> defInterest = (TaleDef def) => (
				from t in this.tales
				where t.def == def
				select t).Sum((Tale t) => t.InterestLevel);
			using (IEnumerator<TaleDef> enumerator = (
				from def in DefDatabase<TaleDef>.AllDefs

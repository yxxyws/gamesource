...
namespace RimWorld
{
	public class WeatherDecider : IExposable
	{
		...
		{
			WeatherDef weatherDef = DefDatabase<WeatherDef>.AllDefs.RandomElementByWeight((WeatherDef w) => this.CurrentWeatherCommonality(w));
		...
		private float CurrentWeatherCommonality(WeatherDef weather)
		{
			...
			{
				if (Find.MapConditionManager.ActiveConditions.Any((MapCondition x) => !x.RainAllowed))
				{
					return 0f;
				}
			}
		...
		public void LogWeatherChances()
		{
			...
			foreach (WeatherDef current in 
				from w in DefDatabase<WeatherDef>.AllDefs

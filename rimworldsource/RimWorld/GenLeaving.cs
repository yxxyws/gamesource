...
namespace RimWorld
{
	public static class GenLeaving
	{
		...
		{
			if (Game.Mode != GameMode.MapPlaying || mode == DestroyMode.Vanish)
			{
				return;
			}
			...
			else
			{
				...
				if (resourcesLeaveFraction > 0f)
				{
					...
					for (int l = 0; l < list.Count; l++)
					{
						...
						int num = Mathf.RoundToInt((float)thingCount.count * resourcesLeaveFraction);
			...
			while (thingContainer.Count > 0)
			{
				...
				Thing thing3;
				if (!thingContainer.TryDrop(thingContainer[0], list2[num3], ThingPlaceMode.Near, out thing3))
				{

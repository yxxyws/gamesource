...
namespace RimWorld
{
	...
	{
		private const float StunDurationFactor_Standard = 11f;
		...
		public void StunHandlerTick()
		{
			...
			if (this.stunTicksLeft > 0)
			{
				...
				{
					this.moteStun = MoteThrower.MakeStunOverlay(this.parent);
				}
				Pawn pawn = this.parent as Pawn;
		...
		public void Notify_DamageApplied(DamageInfo dinfo, bool affectedByEMP)
		{
			...
			{
				this.StunFor(Mathf.RoundToInt((float)dinfo.Amount * 11f));
			}
			else
			{
				if (dinfo.Def == DamageDefOf.EMP && affectedByEMP)
				{
					...
					else
					{
						...
						MoteThrower.ThrowText(loc, "Adapted".Translate(), Color.white, -1);
					}
				}
			}
		}
		protected void StunFor(int ticks)
		{

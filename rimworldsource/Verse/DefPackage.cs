...
namespace Verse
{
	public class DefPackage
	{
		...
		public void SaveIn(LoadedMod mod)
		{
		...
		public string GetFullFolderPath(LoadedMod mod)
		{
			return Path.GetFullPath(Path.Combine(Path.Combine(mod.RootFolder, "Defs/"), this.relFolder));
		}
		public static string UnusedPackageName(string relFolder, LoadedMod mod)
		{
			string fullPath = Path.GetFullPath(Path.Combine(Path.Combine(mod.RootFolder, "Defs/"), relFolder));
			int num = 1;
			string text;
			do
			{

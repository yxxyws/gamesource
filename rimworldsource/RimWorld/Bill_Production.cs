...
namespace RimWorld
{
	public class Bill_Production : Bill, IExposable
	{
		...
		protected override void DrawConfigInterface(Rect baseRect, Color baseColor)
		{
			...
			GUI.color = baseColor;
			WidgetRow widgetRow = new WidgetRow(baseRect.xMax, baseRect.y + 29f, UIDirection.LeftThenUp, 2000f, 29f);
			if (widgetRow.DoTextButton("Details".Translate() + "...", null, true, false))
			{
				Find.WindowStack.Add(new Dialog_BillConfig(this, ((Thing)this.billStack.billGiver).Position));
			}
			if (widgetRow.DoTextButton(this.repeatMode.GetLabel().PadRight(20), null, true, false))
			{
				BillRepeatModeUtility.MakeConfigFloatMenu(this);
			}
			if (widgetRow.DoIconButton(TexButton.Plus, null))
			{
				if (this.repeatMode == BillRepeatMode.Forever)
				{
			...
			if (widgetRow.DoIconButton(TexButton.Minus, null))
			{
				if (this.repeatMode == BillRepeatMode.Forever)
				{
					this.repeatMode = BillRepeatMode.RepeatCount;

...
namespace RimWorld
{
	public static class NameGenerator
	{
		...
		public static Name GeneratePawnName(Pawn pawn, NameStyle style = NameStyle.Full, string forcedLastName = null)
		{
			...
			{
				if (pawn.RaceProps.nameGenerator != null)
				{
					string name = NameGenerator.GenerateName(pawn.RaceProps.nameGenerator, (string x) => !new NameSingle(x, false).UsedThisGame);
		...
		private static NameTriple GeneratePawnName_Shuffled(Pawn pawn, string forcedLastName = null)
		{
			...
			while (num < 50 && NameUseChecker.AllPawnsNamesEverUsed.Any((NameTriple x) => x.Nick == nick));

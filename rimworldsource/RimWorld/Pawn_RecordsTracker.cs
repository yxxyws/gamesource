...
namespace RimWorld
{
	public class Pawn_RecordsTracker : IExposable
	{
		...
		public void AddTo(RecordDef def, float value)
		{
			...
			{
				DefMap<RecordDef, float> defMap;
				DefMap<RecordDef, float> expr_12 = defMap = this.records;
				float num = defMap[def];
				expr_12[def] = num + Mathf.Round(value);
			}
			else
			{
				...
				DefMap<RecordDef, float> defMap2;
				DefMap<RecordDef, float> expr_43 = defMap2 = this.records;
				float num = defMap2[def];
				expr_43[def] = num + value;
			}
		}

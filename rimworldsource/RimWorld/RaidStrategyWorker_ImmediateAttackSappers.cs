...
namespace RimWorld
{
	public class RaidStrategyWorker_ImmediateAttackSappers : RaidStrategyWorker
	{
		...
		{
			return parms.faction.def.humanlikeFaction && parms.faction.def.techLevel >= TechLevel.Midworld && this.PawnGenOptionsWithSappers(parms.faction).Any<PawnGroupMaker_Normal>() && base.CanUseWith(parms);
		}
		...
		private float CheapestSapperCost(Faction faction)
		{
			...
			{
				foreach (PawnGenOption current2 in 
					from op in current.options
		...
		{
			return 
				from gm in faction.def.pawnGroupMakers.OfType<PawnGroupMaker_Normal>()
		...
		{
			return new LordJob_AssaultColony(parms.faction, true, true, true);
		}
		public override Lord MakeLord(IncidentParms parms, List<Pawn> pawns)
		{
			Lord lord = base.MakeLord(parms, pawns);
			lord.avoidGrid = AvoidGridMaker.MakeAvoidGrid(parms, true);
			return lord;
		}
	}
}

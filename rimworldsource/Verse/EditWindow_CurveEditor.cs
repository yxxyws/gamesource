...
namespace Verse
{
	public class EditWindow_CurveEditor : EditWindow
	{
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			if (widgetRow.DoIconButton(TexButton.CenterOnPointsTex, "Center view around points."))
			{
				this.curve.View.SetViewRectAround(this.curve);
			}
			if (widgetRow.DoIconButton(TexButton.CurveResetTex, "Reset to growth from 0 to 1."))
			{
			...
			if (widgetRow.DoIconButton(TexButton.QuickZoomHor1Tex, "Reset horizontal zoom to 0-1"))
			{
				this.curve.View.rect.xMin = 0f;
				this.curve.View.rect.xMax = 1f;
			}
			if (widgetRow.DoIconButton(TexButton.QuickZoomHor100Tex, "Reset horizontal zoom to 0-100"))
			{
				this.curve.View.rect.xMin = 0f;
				this.curve.View.rect.xMax = 100f;
			}
			if (widgetRow.DoIconButton(TexButton.QuickZoomHor20kTex, "Reset horizontal zoom to 0-20,000"))
			{
				this.curve.View.rect.xMin = 0f;
				this.curve.View.rect.xMax = 20000f;
			}
			if (widgetRow.DoIconButton(TexButton.QuickZoomVer1Tex, "Reset vertical zoom to 0-1"))
			{
				this.curve.View.rect.yMin = 0f;
				this.curve.View.rect.yMax = 1f;
			}
			if (widgetRow.DoIconButton(TexButton.QuickZoomVer100Tex, "Reset vertical zoom to 0-100"))
			{
				this.curve.View.rect.yMin = 0f;
				this.curve.View.rect.yMax = 100f;
			}
			if (widgetRow.DoIconButton(TexButton.QuickZoomVer20kTex, "Reset vertical zoom to 0-20,000"))
			{
				this.curve.View.rect.yMin = 0f;
		...
		private void DoCurveEditor(Rect screenRect)
		{
			...
			SimpleCurveDrawer.DrawCurve(this.curve, screenRect, null, null, default(Rect));
			Vector2 mousePosition = Event.current.mousePosition;
			if (Mouse.IsOver(screenRect))
			{
				Rect rect = new Rect(mousePosition.x + 8f, mousePosition.y + 18f, 100f, 100f);
				Vector2 v = SimpleCurveDrawer.ScreenToCurveCoords(mousePosition, screenRect, this.curve.View.rect);
			...
			if (Mouse.IsOver(screenRect))
			{
				...
				{
					Vector2 mouseCurveCoords = SimpleCurveDrawer.ScreenToCurveCoords(Event.current.mousePosition, screenRect, this.curve.View.rect);
			...
			if (this.draggingPoint != null)
			{
				this.draggingPoint.loc = SimpleCurveDrawer.ScreenToCurveCoords(Event.current.mousePosition, screenRect, this.curve.View.rect);
			...
			if (this.DraggingView)
			{
				if (Event.current.type == EventType.MouseDrag)
				{
					...
					SimpleCurveView expr_621_cp_0 = this.curve.View;
					expr_621_cp_0.rect.x = expr_621_cp_0.rect.x - delta.x * this.curve.View.rect.width * 0.002f;
					SimpleCurveView expr_660_cp_0 = this.curve.View;
					expr_660_cp_0.rect.y = expr_660_cp_0.rect.y + delta.y * this.curve.View.rect.height * 0.002f;
					Event.current.Use();
				}
		...
		{
			EditWindow_CurveEditor.<PointsNearMouse>c__Iterator18E <PointsNearMouse>c__Iterator18E = new EditWindow_CurveEditor.<PointsNearMouse>c__Iterator18E();
			<PointsNearMouse>c__Iterator18E.screenRect = screenRect;
			<PointsNearMouse>c__Iterator18E.<$>screenRect = screenRect;
			<PointsNearMouse>c__Iterator18E.<>f__this = this;
			EditWindow_CurveEditor.<PointsNearMouse>c__Iterator18E expr_1C = <PointsNearMouse>c__Iterator18E;
			expr_1C.$PC = -2;
			return expr_1C;
		}
	}

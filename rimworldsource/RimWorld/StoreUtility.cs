...
namespace RimWorld
{
	public static class StoreUtility
	{
		...
		public static bool IsInValidBestStorage(this Thing t)
		{
			SlotGroup slotGroup = t.Position.GetSlotGroup();
			IntVec3 intVec;
			return slotGroup != null && slotGroup.Settings.AllowedToAccept(t) && !StoreUtility.TryFindBestBetterStoreCellFor(t, null, slotGroup.Settings.Priority, Faction.OfColony, out intVec, false);
		}
				...
				if (slotGroup.Settings.AllowedToAccept(t))
				{
					...
					for (int j = 0; j < count2; j++)
					{
						...
						if (lengthHorizontalSquared <= num)
						{
							...
							{
								if (StoreUtility.NoStorageBlockersIn(intVec2, t))
								{
									if (carrier != null)
									{
										if (!carrier.CanReserve(intVec2, 1))
										{
											goto IL_1D1;
										}
									}
									...
									{
										if (carrier == null || t.Position.CanReach(slotGroup.CellsList[0], PathEndMode.ClosestTouch, TraverseParms.For(carrier, Danger.Deadly, TraverseMode.ByPawn, false)))
										{
											flag = true;
											intVec = intVec2;

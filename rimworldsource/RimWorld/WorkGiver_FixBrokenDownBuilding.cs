...
namespace RimWorld
{
	public class WorkGiver_FixBrokenDownBuilding : WorkGiver_Scanner
	{
		...
		public override bool HasJobOnThing(Pawn pawn, Thing t)
		{
			...
			if (pawn.Faction == Faction.OfColony && !Find.AreaHome[t.Position])
			{
				return false;
			}
			if (!t.IsBrokenDown())
		...
		private Thing FindClosestComponent(Pawn pawn)
		{
			...
			return GenClosest.ClosestThingReachable(pawn.Position, ThingRequest.ForDef(ThingDefOf.Components), PathEndMode.InteractionCell, TraverseParms.For(pawn, pawn.NormalMaxDanger(), TraverseMode.ByPawn, false), 9999f, validator, null, -1, false);
		}
	}
}

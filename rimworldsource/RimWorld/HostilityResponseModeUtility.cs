...
namespace RimWorld
{
	...
	public static class HostilityResponseModeUtility
	{
		...
		public static void DrawResponseButton(Vector2 pos, Pawn pawn)
		{
			...
			if (Widgets.ImageButton(rect, icon))
			{
				pawn.playerSettings.hostilityResponse = HostilityResponseModeUtility.GetNextResponse(pawn);
				SoundDefOf.TickHigh.PlayOneShotOnCamera();
				ConceptDatabase.KnowledgeDemonstrated(ConceptDefOf.HostilityResponse, KnowledgeAmount.Total);
			}
			TutorUIHighlighter.HighlightOpportunity("HostilityResponse", rect);

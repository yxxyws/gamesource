...
namespace RimWorld
{
	public class LordToil_MarriageCeremony : LordToil
	{
		...
		public override void Init()
		{
			...
			{
				if (this.Data.spectateRect.Height > this.Data.spectateRect.Width)
				{
		...
		{
			IntVec3 intVec = this.FianceStandingSpotFor(this.firstPawn);
			IntVec3 intVec2 = this.FianceStandingSpotFor(this.secondPawn);
			return CellRect.FromLimits(Mathf.Min(intVec.x, intVec2.x), Mathf.Min(intVec.z, intVec2.z), Mathf.Max(intVec.x, intVec2.x), Mathf.Max(intVec.z, intVec2.z));
		}
	}
}

...
namespace RimWorld
{
	public abstract class CompPower : ThingComp
	{
		...
		{
			CompPower.<CompGetGizmosExtra>c__Iterator7F <CompGetGizmosExtra>c__Iterator7F = new CompPower.<CompGetGizmosExtra>c__Iterator7F();
			<CompGetGizmosExtra>c__Iterator7F.<>f__this = this;
			CompPower.<CompGetGizmosExtra>c__Iterator7F expr_0E = <CompGetGizmosExtra>c__Iterator7F;
			expr_0E.$PC = -2;
			return expr_0E;
		}
		private void TryManualReconnect()
		{
			...
			if (compPower != null)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					MoteThrower.ThrowMetaPuff(compPower.parent.Position.ToVector3Shifted());
				}

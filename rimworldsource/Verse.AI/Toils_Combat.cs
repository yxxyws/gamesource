...
namespace Verse.AI
{
	public static class Toils_Combat
	{
		...
		public static Toil CastVerb(TargetIndex targetInd)
		{
			Toil toil = new Toil();
			toil.initAction = delegate
			{
				toil.actor.jobs.curJob.verbToUse.TryStartCastOn(toil.actor.jobs.curJob.GetTarget(targetInd), false);
			};
		...
		public static Toil FollowAndMeleeAttack(TargetIndex targetInd, Action hitAction)
		{
			...
			followAndAttack.tickAction = delegate
			{
				...
				{
					if (actor.Position.AdjacentTo8WayOrInside(thing))
					{

...
namespace RimWorld
{
	public static class InfestationCellFinder
	{
		...
		private static float GetScoreAt(IntVec3 cell)
		{
			...
			if (!cell.Roofed() || !cell.GetRoof().isNatural)
			{
				return 0f;
			}
			...
			float num5 = Mathf.Clamp(InfestationCellFinder.GetMountainousnessScoreAt(cell), 0.2f, 1f);
			float num6 = 1f - Mathf.Clamp(InfestationCellFinder.DistToBlocker(cell) / 11f, 0f, 0.6f);
			float num7 = Mathf.InverseLerp(-17f, -7f, temperature);
			float num8 = num2 * num3 * num6 * num5 * num4 * num7;
			num8 = Mathf.Pow(num8, 1.2f);
			if (num8 < 6f)
			{
				return 0f;
			}
		...
		public static void DebugDraw()
		{
			...
			{
				CellRect cellRect = Find.CameraMap.CurrentViewRect;
				...
				for (int k = 0; k < Find.Map.Size.z; k++)
				{
					for (int l = 0; l < Find.Map.Size.x; l++)
					{
						...
						if (cellRect.Contains(intVec))
						{
							...
							if (scoreAt2 > 0f)
							{
								float a = GenMath.LerpDouble(6f, num, 0f, 1f, scoreAt2);
								DebugRender.RenderCell(intVec, SolidColorMaterials.SimpleSolidColorMaterial(new Color(0f, 0f, 1f, a)));
							}
						}
					}
				}
		...
		private static float GetMountainousnessScoreAt(IntVec3 cell)
		{
			...
			for (int i = 0; i < 700; i += 10)
			{
				...
				if (c.InBounds())
				{
					...
					{
						if (c.Roofed() && c.GetRoof().isThickRoof)
						{
							num += 0.5f;
						}
					}
		...
		private static void CalculateTraversalDistancesToUnroofed()
		{
			...
			Dijkstra<Region>.Run(InfestationCellFinder.tempUnroofedRegions, (Region x) => x.Neighbors, (Region a, Region b) => Mathf.Sqrt(a.extentsClose.Center.DistanceToSquared(b.extentsClose.Center)), ref InfestationCellFinder.regionsDistanceToUnroofed);
			InfestationCellFinder.tempUnroofedRegions.Clear();
		}

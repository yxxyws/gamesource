...
namespace RimWorld
{
	public static class PartyUtility
	{
		...
		{
			Predicate<Pawn> validator = (Pawn x) => x.RaceProps.Humanlike && PartyUtility.ShouldPawnKeepPartying(x);
			Pawn result;
			if ((
				from x in Find.MapPawns.SpawnedPawnsInFaction(faction)
				where validator(x)

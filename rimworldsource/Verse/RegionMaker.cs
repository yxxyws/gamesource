...
namespace Verse
{
	public static class RegionMaker
	{
		...
		private static void AddThingsInCellToLister(IntVec3 c)
		{
			...
			{
				if (!RegionMaker.newReg.ListerThings.Contains(list[i]) && list[i].Position == c)
				{
					RegionMaker.newReg.ListerThings.Add(list[i]);
				}
			}
		}
		...
		private static void FillRowFrom(int rootX, int rootZ)
		{
			...
			while (true)
			{
				...
				{
					if (flag2 && RegionMaker.CheckRegionableAndProcessNeighbor(intVec + IntVec3.North, Rot4.North))
					{
			...
			while (true)
			{
				...
				{
					if (flag2 && RegionMaker.CheckRegionableAndProcessNeighbor(intVec2 + IntVec3.North, Rot4.North))
					{
		...
		private static void TryMakePortalSpan(IntVec3 c, Rot4 processingDirection)
		{
			...
			{
				if (processingDirection == Rot4.East)
				{
					edgeSpan = new EdgeSpan(c, SpanDirection.North, 1);
				}
				else
				{
					if (processingDirection == Rot4.North)
					{

	...
	public class Genstep_ElevationFertility : Genstep
	{
		...
		public override void Generate()
		{
			...
			if (Find.MapWorldSquare.hilliness == Hilliness.Mountainous)
			{
				...
				while (random == Find.World.CoastDirectionAt(MapInitData.landingCoords));
			...
			foreach (IntVec3 current in Find.Map.AllCells)
			{
				...
				MapGenFloatGrid expr_2BA = mapGenFloatGrid2 = mapGenFloatGrid;
				IntVec3 c;
				IntVec3 expr_2BF = c = current;
				float num2 = mapGenFloatGrid2[c];
				expr_2BA[expr_2BF] = num2 + moduleBase.GetValue(current);
			}

	...
	public static class CharacterCardUtility
	{
		...
		public static void DrawCharacterCard(Rect rect, Pawn pawn, Action randomizeCallback)
		{
			...
			if (randomizeCallback != null)
			{
				...
				if (Widgets.TextButton(rect6, "Randomize".Translate(), true, false))
				{
			...
			if (!flag && pawn.IsColonist && !pawn.health.Dead)
			{
				...
				if (Widgets.ImageButton(rect7, TexButton.Rename))
				{
			...
			SkillUI.SkillDrawMode drawMode;
			if (Game.Mode == GameMode.MapPlaying)
			{
				drawMode = SkillUI.SkillDrawMode.Gameplay;
			}
			else
			{
				drawMode = SkillUI.SkillDrawMode.Menu;
			}
			SkillUI.DrawSkillsOf(pawn, new Vector2(0f, 35f), drawMode);
			GUI.EndGroup();
			GUI.EndGroup();
		}

...
namespace RimWorld
{
	public static class RCellFinder
	{
		public static IntVec3 RandomWanderDestFor(Pawn pawn, IntVec3 root, float radius, Func<Pawn, IntVec3, bool> validator, Danger maxDanger)
		{
			...
			bool drawDebug = Game.isDebugBuild && DebugViewSettings.drawDestSearch;
			if (drawDebug)
			{
			...
			for (int j = Mathf.Max((int)radius / 3, 6); j >= 1; j--)
			{
				...
				if (region2.extentsClose.ClosestDistSquaredTo(root) > radius * radius)
				{
					if (drawDebug)
					{
						Find.DebugDrawer.FlashCell(region2.extentsClose.Center, 0.36f, "region distance");
					}
				}
				else
				{
		...
		{
			bool flag = Game.isDebugBuild && DebugViewSettings.drawDestSearch;
			if (!c.Walkable())
			{
				if (flag)
				{
			...
			Building edifice2 = c.GetEdifice();
			if (edifice2 != null && edifice2.def.regionBarrier)
			{
				if (flag)
				{
		...
		{
			Region root2;
			if (!CellFinder.TryFindClosestRegionWith(root.GetRegion(), TraverseParms.For(searcher, Danger.Deadly, TraverseMode.ByPawn, false), (Region r) => r.Room.PsychologicallyOutdoors && !r.IsForbiddenEntirely(searcher), 300, out root2))
			{
				result = root;
				return false;
			}
			Region reg = CellFinder.RandomRegionNear(root2, 14, TraverseParms.For(searcher, Danger.Deadly, TraverseMode.ByPawn, false), (Region r) => r.Room.PsychologicallyOutdoors && !r.IsForbiddenEntirely(searcher), searcher);
			return reg.TryFindRandomCellInRegionUnforbidden(searcher, (IntVec3 c) => !c.Roofed(), out result);
		}
		...
		public static bool TryFindDirectFleeDestination(IntVec3 root, float dist, Pawn pawn, out IntVec3 result)
		{
			...
			for (int j = 0; j < 30; j++)
			{
				...
				{
					PawnPath pawnPath = PathFinder.FindPath(pawn.Position, randomCell, pawn, PathEndMode.OnCell);
					try
					{
		...
		public static bool TryFindMarriageSite(Pawn firstFiance, Pawn secondFiance, out IntVec3 result)
		{
			...
			bool enjoyableOutsideForFirstFiance = JoyUtility.EnjoyableOutsideNow(firstFiance);
			bool enjoyableOutsideForSecondFiance = JoyUtility.EnjoyableOutsideNow(secondFiance);
			Predicate<IntVec3> baseValidator = delegate(IntVec3 cell)
			{
				...
				if (cell.GetDangerFor(firstFiance) != Danger.None || cell.GetDangerFor(secondFiance) != Danger.None)
				{
					return false;
				}
				if ((!enjoyableOutsideForFirstFiance || !enjoyableOutsideForSecondFiance) && !cell.Roofed())
				{
					return false;
				}
				if (cell.IsForbidden(firstFiance) || cell.IsForbidden(secondFiance))
				{
					return false;
				}
				if (!firstFiance.CanReserveAndReach(cell, PathEndMode.OnCell, Danger.None, 1))
				{
					return false;
				}
				if (!secondFiance.CanReserveAndReach(cell, PathEndMode.OnCell, Danger.None, 1))
				{
					return false;
				}
			...
			if ((
				from x in Find.ListerBuildings.AllBuildingsColonistOfDef(ThingDefOf.MarriageSpot)
			...
			foreach (CompGatherSpot current in GatherSpotLister.activeSpots.InRandomOrder(null))
			{
				for (int i = 0; i < 10; i++)
				{
					...
					if (baseValidator(intVec) && noMarriageSpotValidator(intVec))
					{
						result = intVec;
						bool result2 = true;
						return result2;
			...
			if (CellFinder.TryFindRandomCellNear(firstFiance.Position, 25, (IntVec3 cell) => baseValidator(cell) && noMarriageSpotValidator(cell), out result))
			{
				return true;
			}
			result = IntVec3.Invalid;
		...
		public static bool TryFindPartySpot(Pawn organizer, out IntVec3 result)
		{
			...
			if ((
				from x in Find.ListerBuildings.AllBuildingsColonistOfDef(ThingDefOf.PartySpot)
				where baseValidator(x.Position)
				select x.Position).TryRandomElement(out result))
			{
				return true;

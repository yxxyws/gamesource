...
namespace RimWorld
{
	public class IncidentWorker_Infestation : IncidentWorker
	{
		...
		{
			IntVec3 intVec;
			return base.StorytellerCanUseNowSub() && HivesUtility.TotalSpawnedHivesCount < 30 && InfestationCellFinder.TryFindCell(out intVec);
		}
		...
		private Hive SpawnHiveCluster(int hiveCount)
		{
			...
			{
				Hive hive2;
				if (hive.TrySpawnChildHive(false, out hive2))
				{

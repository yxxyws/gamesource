...
namespace RimWorld
{
	public class WorkGiver_DoBill : WorkGiver_Scanner
	{
		...
		public override Job JobOnThing(Pawn pawn, Thing thing)
		{
			...
			if (!pawn.CanReach(((Thing)billGiver).InteractionCell, PathEndMode.OnCell, Danger.Some, false, TraverseMode.ByPawn))
			{
				return null;
			}
			Bill bill = null;
		...
		private static bool TryFindBestBillIngredients(Bill bill, Pawn pawn, Thing billGiver, List<ThingAmount> chosen)
		{
			...
			{
				WorkGiver_DoBill.ingredientsOrdered.Add(bill.recipe.ingredients[0]);
			}

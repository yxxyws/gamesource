	...
	public class CompRefuelable : ThingComp
	{
		...
		public override void PostDestroy(DestroyMode mode, bool wasSpawned)
		{
			...
			if (wasSpawned && this.Props.fuelFilter.AllowedDefCount == 1)
			{
				...
				while (i > 0)
				{
					...
					i -= thing.stackCount;
					GenPlace.TryPlaceThing(thing, this.parent.Position, ThingPlaceMode.Near);
				}
			}
		}
		public override string CompInspectStringExtra()
		{
			...
			if (!this.Props.consumeFuelOnlyWhenUsed && this.HasFuel)
			{
				...
				text = text + " (" + numTicks.TicksToPeriodString(true) + ")";
			}
			return text;
		}
		...
		{
			CompRefuelable.<CompGetGizmosExtra>c__IteratorF1 <CompGetGizmosExtra>c__IteratorF = new CompRefuelable.<CompGetGizmosExtra>c__IteratorF1();
			<CompGetGizmosExtra>c__IteratorF.<>f__this = this;
			CompRefuelable.<CompGetGizmosExtra>c__IteratorF1 expr_0E = <CompGetGizmosExtra>c__IteratorF;
			expr_0E.$PC = -2;
			return expr_0E;
		}
	}

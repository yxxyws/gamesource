...
namespace RimWorld
{
	internal class MedicalRecipesUtility
	{
		...
		{
			return !pawn.Dead && pawn.RaceProps.Humanlike && part.def.spawnThingOnRemoved != null && MedicalRecipesUtility.IsClean(pawn, part);
		}
		public static bool IsClean(Pawn pawn, BodyPartRecord part)
		{
			return !pawn.Dead && !(
				from x in pawn.health.hediffSet.hediffs
				where x.Part == part
				select x).Any<Hediff>();
		}

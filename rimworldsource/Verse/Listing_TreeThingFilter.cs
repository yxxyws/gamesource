...
namespace Verse
{
	public class Listing_TreeThingFilter : Listing_Tree
	{
		...
		private bool indicateSmallVolumes;
		private List<SpecialThingFilterDef> hiddenSpecialFilters;
		public Listing_TreeThingFilter(Rect rect, ThingFilter filter, ThingFilter parentFilter, float labelColumnWidth, bool indicateSmallVolumes) : base(rect, labelColumnWidth)
		{
			this.filter = filter;
			this.parentFilter = parentFilter;
			this.indicateSmallVolumes = indicateSmallVolumes;
		}
		public void DoCategoryChildren(TreeNode_ThingCategory node, int indentLevel, int openMask, bool isRoot = false)
		{
			...
			foreach (ThingDef current4 in node.catDef.childThingDefs)
			{
		...
		private void DoSpecialFilter(SpecialThingFilterDef sfDef, int nestLevel)
		{
			...
			base.DrawLabelLeft("*" + sfDef.LabelCap, sfDef.description, nestLevel);
		...
		{
			if (base.DoOpenCloseWidget(node, indentLevel, openMask))
			{
				ConceptDatabase.KnowledgeDemonstrated(ConceptDefOf.StorageTabCategories, KnowledgeAmount.Total);
			}
			base.DrawLabelLeft(node.LabelCap, node.catDef.description, indentLevel);
			MultiCheckboxState multiCheckboxState = this.filter.AllowanceStateOf(node);
			if (Widgets.MultiCheckbox(new Vector2(this.labelWidth, this.curY), multiCheckboxState, this.lineHeight))
			{
		...
		{
			bool flag = this.indicateSmallVolumes && tDef.IsStuff && tDef.smallVolume;
			string text = tDef.description;
			if (flag)
			{
				text = text + "\n\n" + "ThisIsSmallVolume".Translate();
			}
			base.DrawLabelLeft(tDef.LabelCap, text, nestLevel);
			if (flag)
			{

	...
	public static class Toils_Interpersonal
	{
		...
		public static Toil ConvinceRecruitee(Pawn pawn, Pawn talkee)
		{
			return new Toil
			{
				initAction = delegate
				{
					if (!pawn.interactions.TryInteractWith(talkee, InteractionDefOf.PrisonerFriendlyChat))
					{
						pawn.jobs.curDriver.ReadyForNextToil();
					}
					else
		...
		public static Toil TryRecruit(TargetIndex recruiteeInd)
		{
			...
			toil.initAction = delegate
			{
				...
				if (pawn.mindState.CheckStartMentalStateBecauseRecruitAttempted(actor))
				{
					return;
				}
			...
			toil.initAction = delegate
			{
				...
				if (pawn.Spawned && pawn.Awake() && actor.interactions.TryInteractWith(pawn, InteractionDefOf.TrainAttempt))
				{
					...
					if (Rand.Value < num)
					{
						...
						RelationsUtility.TryDevelopBondRelation(actor, pawn, 0.03f);
					...
					MoteThrower.ThrowText((actor.DrawPos + pawn.DrawPos) / 2f, text, 300);
				}
			};
			toil.defaultCompleteMode = ToilCompleteMode.Delay;

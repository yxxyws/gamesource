...
namespace RimWorld
{
	public class LordToil_ExitMapAndEscortCarriers : LordToil
	{
		...
		private void UpdateTraderDuty(out Pawn trader)
		{
			...
			if (trader != null)
			{
				trader.mindState.duty = new PawnDuty(DutyDefOf.ExitMapNearest);
		...
		private void UpdateCarriersDuties(Pawn trader)
		{
			for (int i = 0; i < this.lord.ownedPawns.Count; i++)
			{
				...
				if (pawn.GetCaravanRole() == TraderCaravanRole.Carrier)
				{
					...
					{
						pawn.mindState.duty = new PawnDuty(DutyDefOf.ExitMapNearest);
		...
		private void UpdateDutyForChattelOrGuard(Pawn p, Pawn trader)
		{
			...
			if (caravanRole == TraderCaravanRole.Chattel)
			{
				...
				{
					if (!this.TryToDefendClosestCarrier(p, 8f))
					{
						p.mindState.duty = new PawnDuty(DutyDefOf.ExitMapNearest);
			...
			{
				if (!this.TryToDefendClosestCarrier(p, 18f))
				{
					if (trader != null)
					{

...
namespace Verse
{
	public class Dialog_InfoCard : Window
	{
		...
		private PawnKindDef PawnKindDef
		{
			get
			{
				if (this.thing != null && this.thing is Pawn)
				{
					return ((Pawn)this.thing).kindDef;
				}
				return null;
			}
		}
		...
		private void Setup()
		{
			...
			ConceptDatabase.KnowledgeDemonstrated(ConceptDefOf.InfoCard, KnowledgeAmount.Total);
		}
		...
		protected void FillCard(Rect cardRect)
		{
			...
			{
				if (this.tab == Dialog_InfoCard.InfoCardTab.Character)
				{
					CharacterCardUtility.DrawCharacterCard(cardRect, (Pawn)this.thing);
				}
				else
				{
					if (this.tab == Dialog_InfoCard.InfoCardTab.Health)
					{
						cardRect.yMin += 8f;
					...
					{
						if (this.tab == Dialog_InfoCard.InfoCardTab.Records)
						{
		...
		private string GetTitle()
		{
			if (this.thing != null)
			{
				return this.thing.LabelBaseCap;
			}
		...
		[DebuggerHidden]
		private IEnumerable<StatDrawEntry> GetBasicStats()
		{
			Dialog_InfoCard.<GetBasicStats>c__Iterator192 <GetBasicStats>c__Iterator = new Dialog_InfoCard.<GetBasicStats>c__Iterator192();
			<GetBasicStats>c__Iterator.<>f__this = this;
			Dialog_InfoCard.<GetBasicStats>c__Iterator192 expr_0E = <GetBasicStats>c__Iterator;
			expr_0E.$PC = -2;
			return expr_0E;
		}
	}

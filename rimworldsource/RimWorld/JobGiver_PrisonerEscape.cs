...
namespace RimWorld
{
	public class JobGiver_PrisonerEscape : ThinkNode_JobGiver
	{
		...
		{
			IntVec3 vec;
			if (this.ShouldStartEscaping(pawn) && ExitUtility.TryFindClosestExitSpot(pawn, out vec, TraverseMode.ByPawn))
			{
		...
		{
			if (!pawn.guest.IsPrisoner || pawn.guest.HostFaction != Faction.OfColony || !pawn.guest.PrisonerIsSecure)
			{
				return false;
			}

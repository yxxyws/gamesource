...
namespace RimWorld
{
	public static class AutoHomeAreaMaker
	{
		...
		{
			return Find.PlaySettings.autoHomeArea && Game.Mode == GameMode.MapPlaying;
		}
		public static void Notify_BuildingSpawned(Thing b)
		{
			if (!AutoHomeAreaMaker.ShouldAdd() || !b.def.building.expandHomeArea || b.Faction != Faction.OfColony)
			{
				return;
			}

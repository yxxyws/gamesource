...
namespace RimWorld
{
	public static class FireUtility
	{
		...
		public static bool FireNearby(this IntVec3 root)
		{
			Region region = root.GetRegion();
			if (FireUtility.RegionContainsFireNear(region, root))
			{
				return true;
			}
			for (int i = 0; i < region.links.Count; i++)
			{
				...
				for (int j = 0; j < 2; j++)
				{
					if (regionLink.regions[j] != null && regionLink.regions[j] != region && regionLink.regions[j].valid)
					{
						Region r = regionLink.regions[j];
						if (FireUtility.RegionContainsFireNear(r, root))
						{
							return true;
						}
					}
		...
		private static bool RegionContainsFireNear(Region r, IntVec3 root)
		{
			List<Thing> list = r.ListerThings.ThingsOfDef(ThingDefOf.Fire);
			for (int i = 0; i < list.Count; i++)
			{
				if (list[i].Position.InHorDistOf(root, 5f))
				{
					return true;
				}
			}

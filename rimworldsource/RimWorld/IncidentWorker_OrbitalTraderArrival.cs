...
namespace RimWorld
{
	public class IncidentWorker_OrbitalTraderArrival : IncidentWorker
	{
		public override bool TryExecute(IncidentParms parms)
		{
			...
			TraderKindDef def;
			if ((
				from x in DefDatabase<TraderKindDef>.AllDefs
				where x.orbital
				select x).TryRandomElement(out def))
			{
				TradeShip tradeShip = new TradeShip(def);
				if (Find.ListerBuildings.allBuildingsColonist.Any((Building b) => b.def == ThingDefOf.CommsConsole && b.GetComp<CompPowerTrader>().PowerOn))
				{

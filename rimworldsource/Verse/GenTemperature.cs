...
namespace Verse
{
	public static class GenTemperature
	{
		...
		{
			return TemperatureTuning.SeasonalTempVariationCurve.Evaluate(Current.World.DistanceFromEquatorNormalized(worldCoords));
		}
		...
		public static void PushHeat(Thing t, float energy)
		{
			...
			{
				IntVec3 c;
				if (GenAdj.TryFindRandomWalkableAdjacentCell8Way(t, out c))
				{
		...
		public static void DebugLogTemps()
		{
			...
			stringBuilder.AppendLine("Equatorial distance: " + Current.World.DistanceFromEquatorNormalized(Find.Map.WorldCoords));
			stringBuilder.AppendLine();

...
namespace RimWorld
{
	public class Pawn_StoryTracker : IExposable
	{
		...
		public IEnumerable<Backstory> AllBackstories
		{
			get
			{
				Pawn_StoryTracker.<>c__Iterator9A <>c__Iterator9A = new Pawn_StoryTracker.<>c__Iterator9A();
				<>c__Iterator9A.<>f__this = this;
				Pawn_StoryTracker.<>c__Iterator9A expr_0E = <>c__Iterator9A;
				expr_0E.$PC = -2;
				return expr_0E;
			}
		}
		public IEnumerable<WorkTypeDef> DisabledWorkTypes
		{
			get
			{
				Pawn_StoryTracker.<>c__Iterator9B <>c__Iterator9B = new Pawn_StoryTracker.<>c__Iterator9B();
				<>c__Iterator9B.<>f__this = this;
				Pawn_StoryTracker.<>c__Iterator9B expr_0E = <>c__Iterator9B;
				expr_0E.$PC = -2;
				return expr_0E;
			}
		}
		...
		public IEnumerable<WorkTags> DisabledWorkTags
		{
			get
			{
				Pawn_StoryTracker.<>c__Iterator9C <>c__Iterator9C = new Pawn_StoryTracker.<>c__Iterator9C();
				<>c__Iterator9C.<>f__this = this;
				Pawn_StoryTracker.<>c__Iterator9C expr_0E = <>c__Iterator9C;
				expr_0E.$PC = -2;
				return expr_0E;
			}
		}
		...
		private int FinalLevelOfSkill(SkillDef sk)
		{
			...
			if (sk.definedInBackstories)
			{
				...
				foreach (Backstory current in 
					from bs in this.AllBackstories
					where bs != null
					select bs)
				{
		...
		public bool OneOfWorkTypesIsDisabled(IEnumerable<WorkTypeDef> worktypeList)
		{
			foreach (WorkTypeDef current in worktypeList)
			{
				if (this.WorkTypeIsDisabled(current))
				{
					return true;
				}
			}

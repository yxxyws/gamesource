...
namespace RimWorld
{
	public class Designator_Build : Designator_Place
	{
		...
		public override bool Visible
		{
			get
			{
				...
				if (this.entDef.researchPrerequisites != null)
				{
					...
					{
						if (!Find.ResearchManager.IsFinished(this.entDef.researchPrerequisites[i]))
						{
							return false;
						}
					}
		...
		{
			Designator_Build <>f__this = this;
			this.entDef = entDef;
			LongEventHandler.ExecuteWhenFinished(delegate
			{
				<>f__this.icon = entDef.uiIcon;
				ThingDef thingDef = entDef as ThingDef;
				if (thingDef != null)
				{
					<>f__this.iconProportions = thingDef.graphicData.drawSize;
					<>f__this.iconDrawScale = GenUI.IconDrawScale(thingDef);
				}
				else
				{
					<>f__this.iconProportions = new Vector2(1f, 1f);
					<>f__this.iconDrawScale = 1f;
				}
				TerrainDef terrainDef = entDef as TerrainDef;
				if (terrainDef != null)
				{
					<>f__this.iconTexCoords = new Rect(0f, 0f, Designator_Build.TerrainTextureCroppedSize.x / (float)<>f__this.icon.width, Designator_Build.TerrainTextureCroppedSize.y / (float)<>f__this.icon.height);
				}
				if (thingDef != null && thingDef.MadeFromStuff)
				{
					<>f__this.stuffDef = GenStuff.DefaultStuffFor(thingDef);
				}
			});
		}
		...
		public override void DesignateSingleCell(IntVec3 c)
		{
			...
			if (this.entDef == ThingDefOf.OrbitalTradeBeacon)
			{

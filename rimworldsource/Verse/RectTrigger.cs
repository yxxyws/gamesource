...
namespace Verse
{
	public class RectTrigger : Thing
	{
		...
		{
			if (this.destroyIfUnfogged && !this.rect.Center.Fogged())
			{
			...
			if (this.IsHashIntervalTick(60))
			{
				for (int i = this.rect.minZ; i <= this.rect.maxZ; i++)
				{
					for (int j = this.rect.minX; j <= this.rect.maxX; j++)
					{
						...
						{
							if (thingList[k].def.category == ThingCategory.Pawn && thingList[k].def.race.intelligence == Intelligence.Humanlike && thingList[k].Faction == Faction.OfColony)
							{

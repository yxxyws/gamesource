...
namespace Verse
{
	public class Pawn_CarryTracker : IExposable, IThingContainerOwner
	{
		...
		public bool TryDropCarriedThing(IntVec3 dropLoc, ThingPlaceMode mode, out Thing resultingThing)
		{
			if (this.container.TryDrop(this.CarriedThing, dropLoc, mode, out resultingThing))
			{
				if (this.pawn.Faction.HostileTo(Faction.OfColony))
				{
		...
		public bool TryDropCarriedThing(IntVec3 dropLoc, int count, ThingPlaceMode mode, out Thing resultingThing)
		{
			if (this.container.TryDrop(this.CarriedThing, dropLoc, mode, count, out resultingThing))
			{
				if (this.pawn.Faction.HostileTo(Faction.OfColony))
				{
					resultingThing.SetForbidden(true, false);

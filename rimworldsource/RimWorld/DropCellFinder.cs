...
namespace RimWorld
{
	public static class DropCellFinder
	{
		...
		{
			Building building = Find.ListerBuildings.AllBuildingsColonistOfDef(ThingDefOf.OrbitalTradeBeacon).FirstOrDefault((Building b) => !Find.RoofGrid.Roofed(b.Position) && DropCellFinder.AnyAdjacentGoodDropSpot(b.Position, false, false));
			IntVec3 position;
			if (building != null)
			{
			...
			list.AddRange(Find.ListerBuildings.AllBuildingsColonistOfDef(ThingDefOf.OrbitalTradeBeacon));
			list.AddRange(Find.ListerBuildings.AllBuildingsColonistOfDef(ThingDefOf.CommsConsole));
		...
		private static bool IsGoodDropSpot(IntVec3 c, bool allowFogged, bool canRoofPunch)
		{
			...
			if (Game.Mode == GameMode.MapPlaying && !allowFogged && c.Fogged())
			{
				return false;
			}

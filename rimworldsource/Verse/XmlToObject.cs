...
namespace Verse
{
	public static class XmlToObject
	{
		...
		private static HashSet<string> loadedFieldNames = new HashSet<string>();
		public static T ObjectFromXml<T>(XmlNode xmlRoot, bool doPostLoad) where T : new()
		{
			...
			else
			{
				...
				XmlToObject.loadedFieldNames.Clear();
				foreach (XmlNode xmlNode in xmlRoot.ChildNodes)
				{
					if (!(xmlNode is XmlComment))
					{
						if (XmlToObject.loadedFieldNames.Contains(xmlNode.Name))
						{
							Log.Error(string.Concat(new object[]
							{
								"XML ",
						...
						{
							XmlToObject.loadedFieldNames.Add(xmlNode.Name);

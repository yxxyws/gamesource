	...
	public class InteractionWorker_MarriageProposal : InteractionWorker
	{
		...
		public override void Interacted(Pawn initiator, Pawn recipient, List<RulePackDef> extraSentencePacks)
		{
			...
			if (flag)
			{
				...
				initiator.needs.mood.thoughts.RemoveSocialThoughts(ThoughtDefOf.RejectedMyProposal, recipient);
				recipient.needs.mood.thoughts.RemoveSocialThoughts(ThoughtDefOf.RejectedMyProposal, initiator);
				initiator.needs.mood.thoughts.RemoveSocialThoughts(ThoughtDefOf.IRejectedTheirProposal, recipient);
				recipient.needs.mood.thoughts.RemoveSocialThoughts(ThoughtDefOf.IRejectedTheirProposal, initiator);
				extraSentencePacks.Add(RulePackDefOf.Sentence_MarriageProposalAccepted);
			}
			else
			{
				Thought_SocialMemory thought_SocialMemory = (Thought_SocialMemory)ThoughtMaker.MakeThought(ThoughtDefOf.RejectedMyProposal);
				thought_SocialMemory.SetOtherPawn(recipient);
				initiator.needs.mood.thoughts.TryGainThought(thought_SocialMemory);
				Thought_SocialMemory thought_SocialMemory2 = (Thought_SocialMemory)ThoughtMaker.MakeThought(ThoughtDefOf.IRejectedTheirProposal);

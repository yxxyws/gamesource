...
namespace RimWorld
{
	public abstract class JoyGiver_InteractBuilding : JoyGiver
	{
		...
		private Thing FindBestGame(Pawn pawn, bool inBed, IntVec3 partySpot)
		{
			...
			return GenClosest.ClosestThing_Global_Reachable(pawn.Position, thingsOfRequiredThingDefs, PathEndMode.OnCell, TraverseParms.For(pawn, Danger.Deadly, TraverseMode.ByPawn, false), 9999f, validator, null);
		}
		...
		protected virtual Job TryGivePlayJobWhileInBed(Pawn pawn, Thing bestGame)
		{
			...
			return new Job(this.def.jobDef, bestGame, layingDownBed.Position, layingDownBed);
		}
	}
}

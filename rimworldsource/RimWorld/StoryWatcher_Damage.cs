...
namespace RimWorld
{
	public class StoryWatcher_Damage : IExposable
	{
		...
		public void Notify_DamageTaken(Thing damagee, DamageInfo dinfo)
		{
			if (!dinfo.Def.harmsHealth || damagee.Faction != Faction.OfColony)
			{
				return;
			}
			float num = (float)dinfo.Amount;
			if (damagee.def.category == ThingCategory.Pawn && damagee.def.race.Humanlike)
			{
				num *= 5f;
			}
			this.recentDamage += num;
			this.everDamage += num;

...
namespace Verse.AI
{
	public abstract class JobDriver : IJobEndable, IExposable
	{
		...
		protected string ReportStringProcessed(string str)
		{
			...
			{
				str = str.Replace("TargetA", curJob.targetA.Thing.LabelBaseShort);
			}
			else
			{
			...
			{
				str = str.Replace("TargetB", curJob.targetB.Thing.LabelBaseShort);
			}
			else
			{
			...
			{
				str = str.Replace("TargetC", curJob.targetC.Thing.LabelBaseShort);
			}
			else
			{
			...
			try
			{
				...
				else
				{
					if (!this.CheckCurrentToilEndOrFail())
					{
						...
						{
							if (this.curToilCompleteMode == ToilCompleteMode.FinishedBusy && !this.pawn.stances.FullBodyBusy)
							{
								this.ReadyForNextToil();
								return;
							}
						...
						{
							if (this.curToilCompleteMode == ToilCompleteMode.Instant && this.debugTicksSpentThisToil > 300)
							{
								Log.Error(string.Concat(new object[]
								{
									this.pawn,
		...
		public void Notify_PatherArrived()
		{
		...
		public void Notify_PatherFailed()
		{
		...
		public virtual bool TryGetCarriedThingDrawPos(out Vector2 drawPos)
		{

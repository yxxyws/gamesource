...
namespace RimWorld
{
	internal static class BeachMaker
	{
		...
		{
			Rot4 a = Find.World.CoastDirectionAt(MapInitData.landingCoords);
			if (!a.IsValid)
			{
		...
		public static TerrainDef BeachTerrainAt(IntVec3 loc)
		{
			...
			if (value < 0.1f)
			{
				return TerrainDef.Named("WaterDeep");
			}
			if (value < 0.45f)
			{
				return TerrainDef.Named("WaterShallow");
			}
			if (value < 1f)
			{
				return TerrainDef.Named("Sand");
			}
			return null;
		}
	}

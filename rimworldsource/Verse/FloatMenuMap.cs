...
namespace Verse
{
	public class FloatMenuMap : FloatMenu
	{
		...
		public FloatMenuMap(List<FloatMenuOption> options, string title, Vector3 clickPos) : base(options, title, false, false)
		{
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			List<FloatMenuOption> curOpts = FloatMenuMaker.ChoicesAtFor(this.clickPos, pawn);
		...
		private static bool StillValid(FloatMenuOption opt, List<FloatMenuOption> curOpts)
		{
			...
			else
			{
				...
				List<FloatMenuOption> list = FloatMenuMaker.ChoicesAtFor(opt.revalidateClickTarget.Position.ToVector3Shifted(), Find.Selector.SingleSelectedThing as Pawn);

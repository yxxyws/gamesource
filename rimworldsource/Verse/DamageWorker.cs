...
namespace Verse
{
	public class DamageWorker
	{
		...
		public virtual void ExplosionStart(Explosion explosion, List<IntVec3> cellsToAffect)
		{
			...
			MoteThrower.ThrowStatic(explosion.Position, ThingDefOf.Mote_ExplosionFlash, explosion.radius * 6f);
			float magnitude = (explosion.Position.ToVector3() - Find.CameraMapGameObject.transform.position).magnitude;
			Find.CameraMap.shaker.DoShake(4f * explosion.radius / magnitude);
		...
		protected virtual void ExplosionVisualEffectCenter(Explosion explosion)
		{
			for (int i = 0; i < 4; i++)
			{
				MoteThrower.ThrowSmoke(explosion.Position.ToVector3Shifted() + Gen.RandomHorizontalVector(explosion.radius * 0.7f), explosion.radius * 0.6f);
			}
			if (this.def.explosionInteriorMote != null)
			{
				...
				for (int j = 0; j < num; j++)
				{
					MoteThrower.ThrowExplosionInteriorMote(explosion.Position.ToVector3Shifted() + Gen.RandomHorizontalVector(explosion.radius * 0.7f), this.def.explosionInteriorMote);
				}
			}
		}
		public virtual void ExplosionAffectCell(Explosion explosion, IntVec3 c, List<Thing> damagedThings, bool canThrowMotes)
		{
			...
			if (this.def.explosionCellMote != null && canThrowMotes)
			{
				...
				Color color = Color.Lerp(this.def.explosionCellMote.mote.explosionColorStart, this.def.explosionCellMote.mote.explosionColorEnd, t);
				MoteThrower.ThrowExplosionCell(c, this.def.explosionCellMote, color);
			}

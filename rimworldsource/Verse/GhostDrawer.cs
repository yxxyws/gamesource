		...
		private static Graphic GhostGraphicFor(Graphic baseGraphic, ThingDef thingDef, Color ghostCol)
		{
			...
			if (!GhostDrawer.ghostGraphics.TryGetValue(num, out graphic))
			{
				...
				else
				{
					if (baseGraphic != null)
					{
						graphic = GraphicDatabase.Get(baseGraphic.GetType(), baseGraphic.path, ShaderDatabase.Transparent, baseGraphic.drawSize, ghostCol, Color.white);
					}
					else
					{
						graphic = GraphicDatabase.Get(thingDef.graphic.GetType(), thingDef.graphic.path, ShaderDatabase.Transparent, thingDef.graphic.drawSize, ghostCol, Color.white);
					}
				}

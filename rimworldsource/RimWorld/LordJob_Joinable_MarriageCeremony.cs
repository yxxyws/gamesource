...
namespace RimWorld
{
	public class LordJob_Joinable_MarriageCeremony : LordJob_VoluntarilyJoinable
	{
		...
		public override StateGraph CreateGraph()
		{
			...
			lordToil_Party.AddFailCondition(() => this.LostReferenceToFiance());
			...
			lordToil_MarriageCeremony.AddFailCondition(() => this.LostReferenceToFiance());
			stateGraph.AddToil(lordToil_MarriageCeremony);
			LordToil_Party lordToil_Party2 = new LordToil_Party(this.spot);
			lordToil_Party2.AddFailCondition(() => this.LostReferenceToFiance());
			...
			transition.AddAction(new TransitionAction_Message("MessageMarriageCeremonyStarts".Translate(new object[]
			{
				this.firstPawn.LabelBaseShort,
				this.secondPawn.LabelBaseShort
			...
			transition2.AddAction(new TransitionAction_Message("MessageNewlyMarried".Translate(new object[]
			{
				this.firstPawn.LabelBaseShort,
				this.secondPawn.LabelBaseShort
			...
			transition3.AddAction(new TransitionAction_Message("MessageMarriageCeremonyCalledOff".Translate(new object[]
			{
				this.firstPawn.LabelBaseShort,
				this.secondPawn.LabelBaseShort
			...
			transition4.AddAction(new TransitionAction_Message("MessageMarriageCeremonyAfterPartyFinished".Translate(new object[]
			{
				this.firstPawn.LabelBaseShort,
				this.secondPawn.LabelBaseShort
			}), MessageSound.Benefit, this.firstPawn));
			...
			transition5.AddAction(new TransitionAction_Message("MessageMarriageCeremonyCalledOff".Translate(new object[]
			{
				this.firstPawn.LabelBaseShort,
				this.secondPawn.LabelBaseShort
			}), MessageSound.Negative, this.spot));
			...
			transition6.AddAction(new TransitionAction_Message("MessageMarriageCeremonyCalledOff".Translate(new object[]
			{
				this.firstPawn.LabelBaseShort,
				this.secondPawn.LabelBaseShort
			}), MessageSound.Negative, this.spot));
		...
		private bool LostReferenceToFiance()
		{
		...
		private void AddAttendedWeddingThoughts()
		{
			...
			for (int i = 0; i < ownedPawns.Count; i++)
			{
				if (ownedPawns[i] != this.firstPawn && ownedPawns[i] != this.secondPawn)
				{
					...
					{
						ownedPawns[i].needs.mood.thoughts.TryGainThought(ThoughtDefOf.AttendedWedding);
					}
				}
			}
		}

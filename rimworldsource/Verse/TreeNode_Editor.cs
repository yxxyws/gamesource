...
namespace Verse
{
	public class TreeNode_Editor : TreeNode
	{
		...
		private void InitiallyCacheData()
		{
			...
			{
				if (this.ObjectType.IsValueEditable())
				{
		...
		public void RebuildChildNodes()
		{
			...
			else
			{
				foreach (FieldInfo current in 
					from f in this.obj.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
		...
		public void DoSpecialPreElements(Listing_TreeDefs listing)
		{
			...
			if (editable != null)
			{
				...
				{
					listing.DrawInfoText(current, this.nestDepth);
				}

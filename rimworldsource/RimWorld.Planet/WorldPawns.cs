...
namespace RimWorld.Planet
{
	public class WorldPawns : IExposable
	{
		...
		public void PassToWorld(Pawn pawn, PawnDiscardDecideMode discardMode = PawnDiscardDecideMode.Decide)
		{
			...
			{
				Log.Error("Tried to pass a discarded pawn " + pawn + " to world with discardMode=Keep. Pawns destroyed with discard mode should never be stored in WorldPawns.");
		...
		{
			return 
				from x in this.AllPawnsAliveOrDead
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				if (worldPawnSituation != WorldPawnSituation.None)
				{
					...
					foreach (Pawn current in 
						from x in this.GetPawnsBySituation(worldPawnSituation)
		...
		private void AddPawn(Pawn p)
		{
			...
			if ((p.Faction == null || p.Faction.def == FactionDefOf.Colony) && p.RaceProps.Humanlike)
			{

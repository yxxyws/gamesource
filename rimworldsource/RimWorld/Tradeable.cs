	...
	public class Tradeable
	{
		...
		public virtual float ListOrderPriority
		{
			get
			{
				...
				{
					if (this.ThingDef == ThingDefOf.Gold)
					{
						num = 99;
					}
					else
					{
						...
						{
							if (this.ThingDef.IsApparel)
							{
								num = 80;
							}
							else
							{
								if (this.ThingDef.IsRangedWeapon)
								{
									num = 70;
								}
								else
								{
									if (this.ThingDef.IsMeleeWeapon)
									{
										num = 60;
									}
									else
									{
										if (this.ThingDef.isBodyPartOrImplant)
										{
											num = 50;
										}
										else
										{
											if (this.ThingDef.CountAsResource)
											{
												num = -10;
											}
											else
		...
		public bool TraderWillTrade
		{
			get
			{
				return TradeSession.trader.TraderKind.WillTrade(this.AnyThing);
			}
		}
		...
		public float PriceFor(TradeAction action)
		{
			...
			if (action == TradeAction.PlayerBuys)
			{
				num5 = this.BaseMarketValue * (1f - TradeSession.playerNegotiator.GetStatValue(StatDefOf.TradePriceImprovement, true)) * num3 * num * num2;
				num5 = Mathf.Max(num5, 0.5f);
			}
			else
			{
				num5 = this.BaseMarketValue * Find.Storyteller.difficulty.baseSellPriceFactor * this.AnyThing.GetStatValue(StatDefOf.SellPriceFactor, true) * (1f + TradeSession.playerNegotiator.GetStatValue(StatDefOf.TradePriceImprovement, true)) * num3 * num * num2;
				num5 *= Tradeable.LaunchPricePostFactorCurve.Evaluate(num5);
				num5 = Mathf.Max(num5, 0.01f);
				if (num5 >= this.PriceFor(TradeAction.PlayerBuys))
				{
					Log.ErrorOnce("Skill of negotitator trying to put sell price above buy price.", 65387);
					num5 = this.PriceFor(TradeAction.PlayerBuys);
				}
			}
			if (num5 > 99.5f)
			{
		...
		public virtual void ResolveTrade()
		{
			if (this.ActionToDo == TradeAction.PlayerSells)
			{
				...
				while (i > 0)
				{
					...
					thing2.PreSold(TradeSession.playerNegotiator, TradeSession.trader);
		...
		private void CheckTeachOpportunity(Thing boughtThing)
		{
			...
			{
				ConceptDecider.TeachOpportunity(building.def.building.boughtConceptLearnOpportunity, OpportunityType.GoodToKnow);
			}
		}

...
namespace RimWorld
{
	public class IncidentWorker_AgentRevealed : IncidentWorker
	{
		...
		{
			return Find.MapPawns.FreeColonistsCount > 7 && (
				from p in Find.MapPawns.FreeColonists
				where this.CanBeAgent(p)
				select p).Count<Pawn>() > 6 && this.FindAgentFaction() != null;
		}
		private bool CanBeAgent(Pawn p)
		{
			return p.Faction == Faction.OfColony && !p.Downed && p.Awake() && p.health.summaryHealth.SummaryHealthPercent > 0.6f;
		}
		private Faction FindAgentFaction()
		{
			IEnumerable<Faction> source = 
				from fac in Find.FactionManager.AllFactions
				where fac.def.humanlikeFaction && fac.HostileTo(Faction.OfColony)
				select fac;
		...
		public override bool TryExecute(IncidentParms parms)
		{
			...
			list.Add(pawn);
			LordJob_ExitMapNearest lordJob = new LordJob_ExitMapNearest(LocomotionUrgency.Jog);

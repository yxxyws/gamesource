...
namespace RimWorld
{
	public class ITab_Pawn_Needs : ITab
	{
		...
		private static List<ThoughtDef> thoughtTypesPresent = new List<ThoughtDef>();
		...
		private void DrawThoughtListing(Rect listingRect)
		{
			...
			ITab_Pawn_Needs.thoughtTypesPresent.Clear();
			ITab_Pawn_Needs.thoughtTypesPresent.AddRange(
				from th in base.SelPawn.needs.mood.thoughts.DistinctThoughtDefs
				orderby base.SelPawn.needs.mood.thoughts.MoodOffsetOfThoughtGroup(th) descending
				select th);
			float height = (float)ITab_Pawn_Needs.thoughtTypesPresent.Count * 24f;
			Widgets.BeginScrollView(listingRect, ref this.thoughtScrollPosition, new Rect(0f, 0f, listingRect.width - 16f, height));
			Text.Anchor = TextAnchor.MiddleLeft;
			float num = 0f;
			for (int i = 0; i < ITab_Pawn_Needs.thoughtTypesPresent.Count; i++)
			{
				Rect rect = new Rect(0f, num, listingRect.width - 16f, 20f);
				if (this.DrawThoughtGroup(rect, ITab_Pawn_Needs.thoughtTypesPresent[i]))
				{
					num += 24f;
				}
			}
		...
		private bool DrawThoughtGroup(Rect rect, ThoughtDef def)
		{
			try
			{
				...
				if (def.DurationTicks > 5)
				{
					...
					if (thought_Memory != null)
					{
						if (list.Count == 1)
						{
							...
							{
								(def.DurationTicks - thought_Memory.age).TicksToPeriodString(true)
							}));
						}
						else
						{
							...
							{
								(def.DurationTicks - thought_Memory2.age).TicksToPeriodString(true)
							}));
						}
					}
				...
				float num2 = base.SelPawn.needs.mood.thoughts.MoodOffsetOfThoughtGroup(def);
				if (num2 == 0f)
				{

...
namespace RimWorld.Planet
{
	public static class WorldGenerator_Grid
	{
		...
		public static void GenerateGridIntoCurrentWorld(string seedString)
		{
			Rand.Seed = GenText.StableStringHash(seedString);
			Current.World.grid = new WorldGrid();
			NoiseDebugUI.RenderSize = Current.World.Size;
			float num = 1f / ((float)Current.World.Size.x / 200f);
				...
				if (Rand.Value < 0.5f)
				{
					...
					else
					{
						...
						else
						{
							...
							{
								moduleBase3 = new Rotate(0.0, 90.0, 0.0, moduleBase3);
								moduleBase3 = new Translate(0.0, 0.0, (double)(-(double)Current.World.Size.z), moduleBase3);
							}
						}
					}
			...
			foreach (IntVec2 current in Current.World.AllSquares)
			{
				WorldSquare sqDef = WorldGenerator_Grid.GenerateWorldSquareFor(current);
				Current.World.grid.Set(current, sqDef);
			}
		...
		private static WorldSquare GenerateWorldSquareFor(IntVec2 wc)
		{
			...
			{
				if (worldSquare.elevation < 1500f)
				{
